...
def load_data():
    data = {}
    files = {"genes":"outputs/layer1_bbs_genes.csv","constrained":"outputs/layer2_constrained_genes.csv","interactions":"outputs/layer3_interactions.csv","pathways":"outputs/layer4_pathway_scores.csv","clinvar":"outputs/layer4b_clinvar_scores.csv","scores":"outputs/layer5_digenic_scores.csv"}
    for key, path in files.items():
        data[key] = pd.read_csv(path) if os.path.exists(path) else pd.DataFrame()
    return data

def load_ai_text():
    path = "outputs/layer6_ai_interpretations.txt"
...
# ── PIPELINE ──
    ...
    for num,name,source,outfile,color in [("1","Gene Harvesting","Open Targets GraphQL","outputs/layer1_bbs_genes.csv","#00f5c4"),("2","Constraint Filtering","gnomAD pLI scores","outputs/layer2_constrained_genes.csv","#6366f1"),("3","PPI Network","STRING DB interactions","outputs/layer3_interactions.csv","#f59e0b"),("4","Pathway Analysis","Reactome co-membership","outputs/layer4_pathway_scores.csv","#00f5c4"),("4b","Clinical Evidence","ClinVar pathogenicity","outputs/layer4b_clinvar_scores.csv","#ef4444"),("5","Digenic Scoring","DiVaS-inspired formula","outputs/layer5_digenic_scores.csv","#f59e0b"),("6","AI Interpretation","Gemini 2.0 Flash","outputs/layer6_ai_summary.csv","#6366f1")]:
